...
export class UserController {
  ...
  static async updateUserIntegrations(req, res) {
    try {
      const { id, salesforceId } = req.body;
      if (!id || !salesforceId) {
        return res.status(400).json({ error: 'Missing id or salesforceId' });
      }

      const updatedUser = await UserManagementService.updateUserIntegrations(
        id,
        salesforceId
      );
